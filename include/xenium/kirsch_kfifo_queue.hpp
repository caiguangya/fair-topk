...
public:
  ...
  template <class BeforeSuccessFunc>
  std::optional<value_type> pop(BeforeSuccessFunc&& beforeSuccessFunc);
...
template <class T, class... Policies>
template <class BeforeSuccessFunc>
auto kirsch_kfifo_queue<T, Policies...>::pop(BeforeSuccessFunc&& beforeSuccessFunc) -> std::optional<value_type> {
  guard_ptr head_old;
  ...
  for (;;) {
    ...
    if (found_idx) {
      ...
      if (unexecuted) { 
        std::forward<BeforeSuccessFunc>(beforeSuccessFunc)();
        unexecuted = false;
      }
      ...
      if (head_old->items()[idx].value.compare_exchange_strong(
        ...
        return traits::get(old_value.get());
      }
    } else {
